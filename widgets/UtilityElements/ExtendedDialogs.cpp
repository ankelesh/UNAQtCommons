...
LoginPassDialog::LoginPassDialog(QString& login_link, QString& pass_link, QWidget* parent)
...
int LoginPassDialog::ascLoginPass(QString& login_link, QString& pass_link)
{
	int temp;
	LoginPassDialog* lpd = new LoginPassDialog(login_link, pass_link);
	temp = lpd->exec();
	lpd->deleteLater();
	return temp;
}
...
void LoginPassDialog::okPressed()
{
	if (!(loginField->text().isEmpty()) && (loginField->text().count() < 20))
	{
		...
		{
			loginLink = loginField->text();
			passLink = passField->text();
			emit done(confirmed);
		}
	}
}
