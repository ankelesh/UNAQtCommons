...
{
	setMinimumHeight(calculateAdaptiveHeight());	//	Now adaptive height is used by default
#ifdef Q_OS_WINCE
...
{
	setMinimumHeight(calculateAdaptiveHeight());
#ifdef Q_OS_WINCE
	setFont(AppFonts->makeFont(0.7));
#endif
}
...
MegaIconButton::MegaIconButton(QString text, QWidget* parent)
	: QPushButton(text, parent), imageCoefficient(0.6)
{
	setMinimumHeight(calculateAdaptiveHeight());	//	Now adaptive height is used by default
#ifdef Q_OS_WINCE
	setFont(AppFonts->makeFont(0.7));
